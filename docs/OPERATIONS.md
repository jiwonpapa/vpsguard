...
`packaging/certbot/vps-guard-deploy-hook`를 Certbot deploy hook으로 설치합니다. hook은 certificate/key public key 일치, 24시간 이상 유효기간과 VPSGuard config를 검사합니다. 이어서 갱신 PEM을 root-owned `/run/vps-guard-tls` 아래 mode `0440`, group `vps-guard` bundle로 원자 준비하고 edge graceful reload를 요청합니다. 새 worker는 certificate·key·SAN·유효기간을 다시 사전검증한 뒤 Pingora listener FD를 인계받고, 기존 worker는 새 연결을 넘긴 채 최대 30초 동안 기존 연결을 drain합니다. 검증이나 stage가 실패하면 기존 worker에는 signal을 보내지 않습니다.

reload 요청 6초 뒤 hook은 edge health를 read-back합니다. 이어서 `VPS_GUARD_TLS_SERVER_NAME`을 SNI로, `VPS_GUARD_TLS_ADDRESS`의 명시적 IP·port로 TLS handshake를 수행해 갱신 파일과 실제 listener leaf의 SHA-256이 정확히 같을 때만 성공합니다. DNS·CDN 경로와 origin listener 검증을 혼합하지 않습니다.
...
`renew --dry-run`으로 검증했고 당시 restart hook과 served certificate fingerprint
read-back을 완료했습니다. 현재 서버는 원본 Nginx topology이며 새 supervisor를
배포하지 않았습니다. 현재 Rust exact 비교가 포함된 graceful hook의 격리 VPS 실제
renewal, 미설정 서버의 신규 발급 보조 apply와 connection drain timeline은 계속
release gate입니다.
...
실제 변경 전에는 운영 record가 아닌 별도 public origin과 DNS-only test hostname을 먼저 준비합니다. test 이름은 `vpsguard-`로 시작하는 별도 prefix를 사용하고, 운영 apex·`www` 등 금지 hostname을 모두 명시합니다. 다음 명령은 token verify와 명시적 record GET만 실행하며 DNS PATCH나 origin firewall 능력을 갖지 않습니다.

```bash
sudo vps-guard provider cloudflare-preflight \
  --config /etc/vps-guard/config.toml \
  --expected-hostname vpsguard-test.example.com \
  --forbid-hostname example.com \
  --forbid-hostname www.example.com \
  --required-hostname-prefix vpsguard-test.
```

이 점검은 금지 hostname이 없거나 prefix가 `vpsguard-`로 시작하지 않으면 API 호출 전에 거부합니다. 이후에도 record가 이미 proxied 상태이거나 effective TTL이 설정 상한보다 크면 변경 전 실패합니다. 출력 JSON에는 token·응답 body·origin address를 넣지 않고 exact record ID·type·proxy 상태·정규화 TTL만 기록합니다.

코드, fake API와 read-only 격리 preflight 검증은 완료됐지만 실제 test zone의 DNS 전환·`cf-ray`·origin lock·복구 증거는 아직 release gate로 남습니다. 현재 `g7devops.com` zone의 운영 record는 test 대상으로 사용하지 않습니다.
...
- `worker_threads`를 생략하면 가용 CPU 기준 1~4개, 명시하면 1~8개를 사용합니다. 2GB·2 vCPU 기준 명시값은 `2`이며 변경 뒤 RSS·처리량을 함께 확인합니다.
...
`vm-load-regression`은 운영 서버가 아니라 manifest의 private libvirt VM만
허용합니다. verified x86_64 release bundle과 고정 SHA-256의 k6 binary를 받아
다음 계약을 자동 실행합니다.

```bash
python3 -m tools.vpsguard_harness vm-load-regression \
  --manifest tests/vm/g7-bench-load-regression.json \
  --bundle /path/to/verified/x86_64/bundle \
  --k6-binary /path/to/k6-v0.55.2-linux-amd64/k6 \
  --evidence target-evidence/nfr001-vm/report.json
...
python3 -m tools.vpsguard_harness vm-load-regression \
  ...
  --k6-binary /path/to/k6-v0.55.2-linux-amd64/k6 \
  --evidence target-evidence/nfr001-vm/report.json \
  --run --confirm isolated-vm:g7-test
```

기본 workload는 `50 VU·15초·100ms think time` 3회이며 측정 순서를
교차합니다. Nginx 정적 경로로 proxy 비용을 분리하되 실제 G7 `/`의 direct·guard
`200` smoke도 요구합니다. 종료 시 Edge PID executable 경계, Nginx full config
hash, public HTTPS 상태, libvirt/guest memory와 host·guest stage 제거를
read-back합니다. 어느 원복 단계든 실패하면 성공 증거를 쓰지 않습니다.
