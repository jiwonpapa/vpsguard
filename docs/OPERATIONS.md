...
현재 `g7devops` 파일럿은 인증서 소유권을 기존 Nginx에 유지하고 애플리케이션 요청만 VPSGuard에 편입합니다.

```text
Internet -> Nginx public TLS -> VPSGuard 127.0.0.1:18080
         -> Nginx origin 127.0.0.1:18081 -> PHP-FPM
```

이는 공개 트래픽이 VPSGuard의 Host·forwarded header·body·보안 header·관찰 계층을 통과하게 하지만, Pingora의 직접 public 80/443 TLS 소유를 증명하지는 않습니다. 직접 TLS는 별도 `EDGE-001`, `EDGE-002` release gate로 유지합니다.

```bash
bash scripts/cutover-g7devops.sh --plan --to-edge
bash scripts/cutover-g7devops.sh --preflight --to-edge \
  target/release-bundle/x86_64-unknown-linux-gnu/vpsguard-<version>

VPS_GUARD_CUTOVER_CONFIRM=g7devops:to-edge:<BUILD-INFO의-git-commit> \
  bash scripts/cutover-g7devops.sh --apply --to-edge \
  target/release-bundle/x86_64-unknown-linux-gnu/vpsguard-<version>
```

전환 후보는 release checksum에 포함됩니다. preflight는 설치 binary hash, 현재 Nginx와 VPSGuard config가 승인된 후보와 정확히 일치하는지, 격리된 Nginx 후보 문법과 공개 HTTPS 기준 응답을 확인합니다. apply는 배포 snapshot과 active Nginx/config backup을 만든 뒤 전환하고 공개 응답의 `x-vps-guard: guard-edge`, loopback ready와 origin listener를 read-back합니다.
...
probe·Nginx 문법·service·header read-back 중 하나라도 실패하면 active Nginx와 VPSGuard config 및 edge 기동 상태를 transaction 직전 값으로 복구합니다. 이 파일럿은 `mode = "observe"`, Cloudflare 비활성, HSTS 비활성과 기존 88MiB Nginx body 한도를 유지합니다.
