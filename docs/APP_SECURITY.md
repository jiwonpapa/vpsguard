...
- Ubuntu 단독 설치의 기본 provider는 Linux-PAM입니다. `pam_authenticate`, `pam_acct_mgmt`와 `vpsguard-admin` group 검사를 모두 통과한 non-root 사용자만 허용합니다. 최초 등록은 root local 단회 코드 뒤 실제 사용자가 웹 QR을 확인해야 하며 installer·test fixture의 선등록과 사용자 home seed 재사용을 금지합니다.
- PAM stack은 서버 비밀번호와 account 상태만 검증하고 root helper가 별도로 TOTP·일회용 복구 코드를 검증합니다. PAM TOTP seed는 root-only master key로 XChaCha20-Poly1305 봉인하며 복구 코드는 keyed hash만 저장합니다.
