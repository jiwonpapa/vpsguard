...
### 다음 구현 배치: 범용·G7 애플리케이션 보안 계층

- 구현 전에 보안 header, 인증 시도 제한과 bounded request signal의 요구사항 ID·오탐 정책·검증 경계를 먼저 정의
- generic core와 G7 overlay를 분리하고 `protocol_only`에는 app 보안 판정을 적용하지 않음
- SQL/XSS payload를 정규식만으로 차단하는 범용 WAF를 약속하지 않고 origin 수정이 필요한 보호와 edge에서 안전한 보호를 구분
