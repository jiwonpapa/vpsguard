...
- [ ] 격리 Ubuntu 24.04 VM 20회 왕복과 100ms public probe timeline 수집
...
### 현재 배치: 직접 관리·standalone 방화벽·AI bot 방어 마감

- 요구사항: `UI-016`, `UI-017`, `SEC-015`~`SEC-017`, `ACT-013`, `ACT-014`, `EDGE-014`, `DET-013`, `NFR-002`, `NFR-014`
- [ ] Apache·Nginx trusted external TLS terminator에서 별도 관리 Host 직접 접속
- [ ] Linux-PAM `vpsguard-admin` allowlist, root·system·잠김·만료 거부와 MFA·복구
- [ ] standalone typed UFW plan·dry-run·apply·read-back·rollback과 JW-agent delegated fail-closed mode
- [ ] limiter capacity fail-open 제거와 IP·prefix·route·global bounded fallback
- [ ] Google·Naver·Bing verified crawler와 미허용 declared AI bot 정책
- [ ] request framing·smuggling 거부와 선택형 ModSecurity·OWASP CRS profile
- [ ] host-to-VM UFW·PAM·crawler·회전 IP·WAF A/B와 2GB soak 증거
- [ ] JW-agent가 소유하는 Nginx·Certbot·service·file·terminal 관리 기능은 VPSGuard에 중복 구현하지 않음
...
- [ ] 격리 Ubuntu VM 20회 왕복과 100ms public probe timeline 수집
