...
### 최근 완료 배치: 운영 지표 도움말과 용어집

- 요구사항: `UI-010`
- [x] RPS·p95 지연에 keyboard 접근 가능한 portal tooltip 제공
- [x] RPS·p95·경로 비용·서비스 압력·load/core·저장 손실·retention·provider stage 정의
- [x] 각 지표의 의미·산정 방식·실제 수집 출처 표시
- [x] 별도 terminal 없이 desktop/mobile navigation에서 용어집 접근
- [x] tooltip clipping과 용어집 내용 Playwright 회귀 통과

### 최근 완료 배치: crawler trust와 shared-IP 만료 경계

- 요구사항: `DET-003`, `DET-009`
- [x] 검색 crawler UA만으로 verified 처리하지 않고 허용 provider·공식 CIDR 일치 강제
- [x] Edge가 검증한 crawler가 관측 window의 과반일 때만 Control trust 신호로 사용
- [x] 소수 verified crawler가 다른 공격 traffic의 자동화 신호를 덮지 못함
- [x] shared-IP client 한도는 다음 1분 window에 자동 해제되고 영구 rule을 만들지 않음
- [ ] signed clearance·origin 인증 session을 aggregate session continuity에 연결하는 실제 NAT browser E2E

### 최근 완료 배치: NFR-001 비교 성능 release gate

- 요구사항: `NFR-001`
- [x] 동일 50 VU·15초·100ms think time workload의 direct-origin과 release guard-edge를 순차 측정
- [x] 실제 k6 0.55 summary 형식의 p95·요청률·실패율 strict parser와 비정상 수치 거부
- [x] p95 추가 지연 2ms·처리량 감소 10% 고정 예산과 초과 시 non-zero release 차단
- [x] commit·Rust·kernel·machine·artifact/config hash와 비교 결과를 bounded JSON으로 보존
- [x] Python 공통 runner·HTTP/1.1 keep-alive fixture·구조화 실패·단위 회귀 적용
- [x] Pingora 기본 worker 1개를 가용 CPU 기준 1~4개 자동·명시값 1~8개로 bounded 설정
- [x] 연결 없는 telemetry payload 생성과 request ID 고정 nonce의 요청별 formatting 제거
- [x] 2026-07-24 로컬 무제한 반복 workload 실패를 정상 browse의 100ms think time 계약 누락으로 식별
- [x] 동일 2GB Ubuntu 서버의 direct Nginx 3회 교차 비교에서 p95 `+1.188ms`, 처리량 `-0.47%`, 실패율 0과 exact 원복 통과
- [x] CI Python origin의 TCP delayed-ACK 약 40ms 왜곡을 `TCP_NODELAY`와 소켓 회귀 테스트로 제거
...
### 최근 완료 배치: Cloudflare 폐쇄루프 자동 증거

- 요구사항: `ACT-006`, `ACT-011`, `SEC-001`, `SEC-004`
- [x] 재개 가능한 단계·부분 변경 즉시 rollback·snapshot 복구 후 exact read-back 회귀
- [x] 401·403·429·5xx 안정 오류 분류와 provider 응답 body·token 비노출
- [x] 고정 zone ID·record ID·동일 hostname allowlist 외 요청의 변경 전 거부
- [x] provider 미설정·실패에서 `LOCAL_GUARD`와 기존 전이 시각을 유지하고 public edge와 분리
- [ ] 전용 test zone의 실제 proxy 전환·TTL drain·origin lock·승인 복구 운영 증거

### 최근 완료 배치: 관리자 UI 알파 계약

- 요구사항: `UI-003`, `UI-007`, `UI-009`, `UI-013`, `UI-014`
- [x] RPS·p95·대역폭·2xx~5xx·처리 중·upstream 연결 집계를 같은 API snapshot으로 표시
- [x] provider 부분 실패와 snapshot 복구 read-back을 사건 timeline에 함께 표시
- [x] 운영 명령의 역할 확인·재확인·CSRF·idempotency browser 회귀
- [x] SSE 단절과 collector delayed·error·오류 code를 정상 상태와 분리
- [x] 공개 메뉴 9개 allowlist와 packet capture·process management 부재 감사
