...
### 다음 구현 배치: protocol-only 통과 계층

- 요구사항: `EDGE-013`
- `profiled`와 별도인 typed `protocol_only` inspection mode
- app profile·행동 판정은 생략하되 TLS·Host·forwarded header·body·timeout·연결 상한과 bounded 계측은 유지
- 지원하지 않는 non-web port는 가로채지 않고, 소유한 HTTP listener의 비HTTP 입력은 명시적으로 거부
