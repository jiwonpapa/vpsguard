...
### 다음 구현 배치: 외부 연동과 핵심 service 관측 경계

- 요구사항: `EDGE-013`, `OBS-011`, `TLS-006`, `NFR-008`
- 수동 HTTP·Redis wire probe를 검증된 client crate 뒤의 typed adapter로 교체
- 관리자 확정 service allowlist와 cgroup v2 resource snapshot 구현
- `profiled`·`protocol_only`를 enforcement mode와 분리하고 protocol별 E2E 추가
- 기존 인증서 manager·Certbot timer를 우선 감지하고, 미설정일 때만 신규 발급·timer·deploy hook을 승인형 plan/apply로 연결
- Cloudflare record ID·type allowlist와 동일 hostname 다중 A/AAAA 처리 후 test zone 검증
