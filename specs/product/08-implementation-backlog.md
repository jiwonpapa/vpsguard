...
### 다음 구현 배치: 로그·분석 저장 계층

- 요구사항: `OBS-002`, `OBS-007`, `SEC-005`, `NFR-003`
- request별 SQLite write를 bounded batch writer로 교체
- 10초·1분 rollup과 detail·client IP·aggregate·incident retention 분리
- queue drop, DB/WAL 크기, retention 성공과 disk budget 관측
- request body·query·cookie·authorization 미저장 회귀 gate
