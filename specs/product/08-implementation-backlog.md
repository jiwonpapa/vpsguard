...
### 다음 구현 배치: 핵심 service cgroup·semantic 관측

- 요구사항: `OBS-004`, `OBS-005`, `OBS-006`, `OBS-011`
- 전체 process 감사를 하지 않고 관리자가 확정한 systemd unit allowlist만 수집
- cgroup v2 CPU·memory·I/O·process/task와 Nginx/Apache·PHP-FPM·MySQL/MariaDB·Redis semantic metric을 분리 표시
- collector timeout·stale·권한 부족을 정상값처럼 표시하지 않는 API·UI·fixture gate
