...
첫 관리자 편집 범위는 재시작이 필요 없는 WATCH·LOCAL_GUARD·EMERGENCY_PROXY의
strict/upload 분당 제한입니다. listener, origin, TLS 파일 경로, PAM group, 방화벽
소유권과 credential 경로는 이 API schema에 포함하지 않고 읽기 전용 설정 inventory로
남깁니다. policy 파일 반영 version과 Edge telemetry에서 다시 관측된 version은 서로
구분해, 아직 Edge에 적용되지 않은 값을 적용 완료로 표시하지 않습니다.
단계별 제한 원본은 Edge schema와 분리된 hash settings sidecar에 저장해 이전 Edge
binary로 rollback해도 policy parser 계약을 깨지 않습니다.
...
- 내비게이션은 모니터링과 운영 업무로 구분하고 mobile에서는 drawer로 전환
- overview는 현재 보호, 실시간 트래픽, 서버 압력, 외부 보호 순서의 명시적 section 사용
- 선택한 대상의 상세 정보가 없을 때 상시 우측 설명 panel로 작업 폭을 축소하지 않음
- 한국어 기본 문구와 숫자 단위 통일
