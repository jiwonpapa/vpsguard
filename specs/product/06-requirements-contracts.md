...
| `UI-015` | 일상 관리 접속은 설정된 typed 인증 provider와 2단계 인증으로 완료하고 SSH·terminal은 최초 등록과 비상 복구에만 사용 | PAM·호환 local provider의 최초 등록, 일반 로그인, 복구 코드 로그인, 로그아웃과 재시작 후 session 복원이 browser/API E2E를 통과 |
...
| `SEC-013` | 관리자 계정에 TOTP 2단계 인증과 일회용 복구 코드를 제공 | local provider TOTP seed는 비밀번호 유래 key, PAM provider seed는 root-only credential key로 AEAD 봉인되고 API·로그·DB 평문에 없으며 복구 코드는 hash만 저장하고 성공 즉시 소비 |
...
| Method | Path | 목적 |
|---|---|---|
| `POST` | `/auth/enrollment` | local 단회 code와 새 관리자 credential로 TOTP 등록 시작 |
| `POST` | `/auth/enrollment/confirm` | TOTP 확인 후 계정·복구 code·첫 session 원자 확정 |
