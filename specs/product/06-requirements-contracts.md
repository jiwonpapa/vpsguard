...
| `OPS-009` | first install과 shadow 배포 전에 배포 소유 상태를 snapshot하고 실패·관리자 요청 시 정확히 원상복귀 | binary·unit·drop-in·config·service enable/active·소유 directory의 기존/부재 상태를 복구하고 SSH·Nginx·인증서·사이트·non-web listener hash 불변과 snapshot checksum을 read-back |
