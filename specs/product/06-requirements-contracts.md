...
| `UI-015` | 일상 관리 접속은 VPSGuard 전용 관리자 계정과 2단계 인증으로 완료하고 SSH·terminal은 최초 등록과 비상 복구에만 사용 | 최초 등록, 일반 로그인, 복구 코드 로그인, 로그아웃과 재시작 후 session 복원이 browser/API E2E를 통과 |
...
| `SEC-012` | VPSGuard 관리자 계정은 Linux·SSH·root 계정과 분리하고 비밀번호는 Argon2id PHC hash로만 저장 | OS 계정 인증 호출과 평문·복호화 가능한 비밀번호가 없고, 미등록 ID도 동일한 password hash 검증 경로와 일반 오류를 사용 |
| `SEC-013` | 관리자 계정에 TOTP 2단계 인증과 일회용 복구 코드를 제공 | TOTP seed는 비밀번호 유래 key로 AEAD 봉인되고 API·로그·DB 평문에 없으며, 복구 코드는 hash만 저장하고 성공 즉시 소비 |
