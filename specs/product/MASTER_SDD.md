...
VPSGuard는 소유한 TCP 80/443 외 listener를 가로채지 않습니다. standalone mode의 UFW 화면은 typed port·IP rule을 제공하지만 현재 SSH·관리 HTTPS와 사용자 기존 rule을 보호하고 VPSGuard 소유 comment가 있는 rule만 변경합니다. JW-agent 위임 mode에서는 UFW를 변경하지 않습니다. SSH, DB, mail, game server와 사용자 정의 port의 트래픽은 VPSGuard를 통과시키지 않습니다. 반대로 VPSGuard가 소유한 80/443에 들어온 비HTTP protocol을 HTTP origin으로 무조건 전달하면 protocol confusion과 우회가 생기므로 거부합니다. 동일 443에서 별도 raw TLS service를 multiplex해야 하는 요구는 명시적 SNI/ALPN L4 listener와 별도 요구사항 없이는 지원하지 않습니다.
...
| `/var/lib/vps-guard/policy.settings.json` | 관리자 단계별 보호 제한과 policy version sidecar | hash·schema 검증, 구버전 Edge와 분리 |
...
11. 거버넌스·fixture·evidence·로컬 오케스트레이션은 Python 표준 라이브러리, privileged production mutation은 Rust typed adapter가 소유하며 Shell은 얇은 호환 진입점으로 제한합니다.
12. 개발·CI Cargo 산출물은 bounded profile과 보존 경계가 있는 정리 하네스로 관리하며 release bundle과 검증 evidence를 캐시와 함께 삭제하지 않습니다.
13. coverage gate는 workspace와 versioned named production file을 함께 검증하며 process·network adapter를 제외해 통과시키지 않습니다.
14. 개발자는 명시한 crate·Python·Web 범위를 빠르게 검사할 수 있지만 merge 증거는 workspace 전체 gate만 인정합니다.
15. 비-merge 변경 커밋과 pull request 본문은 관련 요구사항 ID가 없으면 CI에서 거부합니다.
...
- Ubuntu standalone 기본 인증은 Linux-PAM과 `vpsguard-admin` group allowlist를 사용하며 root·system·잠김·만료 계정을 거부합니다. 호환용 local account provider는 명시적 선택에서만 유지합니다.
- PAM 비밀번호는 verifier를 포함해 저장하지 않고 PAM 인증과 account 상태 검사를 bounded blocking worker에서 수행합니다.
- peer credential이 확인된 local admin socket의 one-time code는 local·PAM 최초 관리자 TOTP 등록과 break-glass 복구에만 사용하고 일상 접속은 관리자 비밀번호와 TOTP를 요구합니다.
- PAM 최초 등록은 기존 서버 계정 비밀번호·group·account 상태를 root helper에서 확인한 뒤 실제 사용자가 웹 QR을 직접 확인해야 완료합니다. installer·test fixture가 운영 seed를 미리 만들거나 사용자 home의 seed를 재사용하면 안 됩니다.
- local TOTP seed는 비밀번호 유래 key, PAM TOTP seed는 root-only credential key로 AEAD 봉인하고 복구 코드는 keyed hash만 저장해 성공 즉시 소비합니다.
