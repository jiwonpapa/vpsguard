...
| `/var/log/vps-guard/` | 구조화 로그와 리포트 | 비밀값 마스킹 |
...
- peer credential이 확인된 local admin socket에서 짧은 one-time 로그인 코드를 발급하고 이후 Secure·HttpOnly·SameSite=Strict session으로 전환합니다.
- 로그인 코드는 session 발급 외 운영 명령에 사용할 수 없고 모든 변경에는 Origin·CSRF·idempotency 검사가 필요합니다.
