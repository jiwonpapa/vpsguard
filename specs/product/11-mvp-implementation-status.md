...
| `TLS-002`, `TLS-005`, `TLS-006` 하네스 | startup cert/key·SAN·유효기간 preflight, 6시간 공개 cert·Certbot renewal/timer 관측, external/assisted/manual 소유권, systemd credential 경계, 승인 전 HTTP-01 plan, 갱신 PEM 원자 stage, 새 worker 사전검증·Pingora listener FD 인계·30초 connection drain과 보존 | typed unit/API/UI/packaging tests; 실제 staging 발급·renew와 supervisor graceful reload VPS 증거는 미수집 |
...
- `TLS-002`~`TLS-006`: 기존 manager·timer 감지, 승인 전 plan, served certificate exact 비교, root-owned runtime stage와 Pingora FD handoff graceful reload는 구현됐으며, plan hash 기반 apply, Certbot staging HTTP-01·systemd timer renew와 실제 무순단 renewal·bypass 후 fingerprint 증거가 남음
