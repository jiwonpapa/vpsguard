...
현재 요구사항 90개 중 `PLANNED` 17개, `CODE_ONLY` 42개, `AUTO_PASS` 31개, `VPS_PASS` 0개입니다. 즉 73개는 코드 또는 계약이 존재하지만 자동 수용 기준까지 통과한 것은 31개이며, 실제 VPS 운영 수용 기준을 완료한 항목은 아직 없습니다.
...
| `OBS-003` 일부, `OBS-005`·`OBS-006` 생존 probe | Linux `/proc`의 load·memory·swap·uptime, Nginx/PHP HTTP 200, MySQL TCP, Redis PING와 collector health | agent tests, control resource API; semantic metric은 미완료 |
| `OBS-007` 일부 | bounded telemetry datagram·4,096 queue, SQLite WAL raw sample과 시간별 detail 삭제 | request별 동기 insert이며 10초·1분 rollup, batch writer, `live_seconds`·`aggregate_days`·`incident_days` 적용과 `events_directory`는 미구현 |
...
| `TLS-002`, `TLS-005` 하네스 | startup cert/key·SAN·현재 유효기간 preflight, cert/key deploy hook, update·uninstall 인증서 보존 | 기존 manager·`certbot.timer` 감지, 만료 임박, served cert 비교와 graceful reload는 미구현 |
...
| 운영 로그 일부 | edge/control JSON stdout 로그를 systemd journal이 수집 | request 완료 `info`에 client IP·path가 포함될 수 있고 per-unit rate limit·보존 상태 UI가 없어 기본 log level·sampling 보강 필요 |
...
- `OBS-003`~`OBS-007`, `OBS-010`, `OBS-011`: 현재 health probe와 서버 전체 일부 값만 존재하며 실제 semantic metric, allowlisted service별 cgroup CPU·memory·I/O·process/task, batch writer·rollup·계층별 retention·disk budget, 2GB VPS 정확도와 route-resource 상관 검증이 남음
...
- `TLS-002`~`TLS-006`: 기존 manager·timer 감지, 미설정일 때만 Certbot 신규 발급 plan/apply, staging HTTP-01, systemd timer renew, 실제 served certificate 비교와 bypass 후 fingerprint 증거
