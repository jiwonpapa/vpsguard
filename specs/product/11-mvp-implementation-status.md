...
현재 요구사항 96개 중 `PLANNED` 13개, `CODE_ONLY` 41개, `AUTO_PASS` 42개, `VPS_PASS` 0개입니다. 즉 83개는 코드 또는 계약이 존재하며 자동 수용 기준까지 통과한 것은 42개이고, 실제 VPS 운영 수용 기준을 완료한 항목은 아직 없습니다.
...
| `OPS-009` 계약 | first install·shadow 배포 전 소유 상태 snapshot, 실패·수동 원상복귀와 protected 경계 read-back | 구현 전 `PLANNED`; `g7devops` apply·restore 증거 없음 |
