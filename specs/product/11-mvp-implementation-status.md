...
| `OPS-010` | 단일 OS operation lock, plan hash, typed 단계 ledger·원자 rollback checkpoint 재개, 실제 deployment restore driver, 5초 public 순단·60초 apply/update·30초 restore·10초 rollback 예산, 실패 자동 rollback | Rust engine·driver fault·process reconstruction tests, site tree 거부와 빠른 restore fixture; direct public ingress driver와 격리 Ubuntu VM 20회 100ms probe timeline은 미수집 |
...
| `NFR-009` | Python 표준 라이브러리 기반 argv runner·구조화 오류·redaction, Rust privileged transaction 경계, first-install restore 실제 driver, 얇은 Shell wrapper와 line-count 비증가 ratchet | Python unit·language policy·docs·requirements·ops harness; 남은 direct public ingress Shell은 별도 Rust driver 후속 배치 |
...
현재 허용 범위는 로컬·CI 검증까지입니다. `g7devops`는 원본 `Nginx public 80/443 -> PHP-FPM` topology로 복구했으며 이번 운영 하네스 변경을 배포하지 않습니다. 이후 원격 preflight·shadow·public apply는 별도 명시적 승인과 격리 VM 왕복 증거 뒤에만 수행합니다.
