...
현재 요구사항 96개 중 `PLANNED` 12개, `CODE_ONLY` 41개, `AUTO_PASS` 43개, `VPS_PASS` 0개입니다. 즉 84개는 코드 또는 계약이 존재하며 자동 수용 기준까지 통과한 것은 43개이고, 실제 VPS 운영 수용 기준을 완료한 항목은 아직 없습니다.
...
| `OPS-009` 하네스 | first install·shadow 배포 전 checksum snapshot, commit-bound apply, stdin root-only token 전달, 실패·수동 원상복귀와 protected SSH·Nginx·인증서·G7·listener 경계 read-back | fixture exact restore·corrupt snapshot·protected drift 자동 테스트; `g7devops` apply·restore 운영 증거는 미수집 |
...
- `OPS-003`~`OPS-007`, `OPS-009`: shadow apply·원상복귀, public cutover·bypass 왕복, update/uninstall 실증, x86_64/aarch64 artifact 실행 smoke
- 2GB `g7devops` 성능·장애·복구 파일럿

## 실행 범위

현재 허용 범위는 로컬·CI와 `g7devops` **read-only preflight 및 명시적 승인 shadow apply**까지입니다. `scripts/deploy-g7devops.sh`는 기본 plan-only이고 apply는 commit-bound 확인과 자동 restore snapshot을 요구합니다. public ingress와 Cloudflare 변경은 위 release gate를 통과한 별도 승인 작업입니다.
