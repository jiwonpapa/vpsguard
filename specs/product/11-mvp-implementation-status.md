...
현재 요구사항 123개 중 `PLANNED` 9개, `CODE_ONLY` 28개, `AUTO_PASS` 71개, `VPS_PASS` 15개입니다. 즉 114개는 코드 또는 계약이 존재하며 자동 수용 기준까지 통과한 것은 86개입니다. `VPS_PASS`는 보존된 운영 증거 수준이며 요구사항 전체의 release 완료를 뜻하지 않습니다.
...
- `OPS-005`, `OPS-006`, `OPS-010`: verified x86_64 bundle의 실제 systemd update·snapshot restore 1회와 자동 fixture는 통과했으며, 실제 uninstall, 격리 Ubuntu VM 20회 apply·restore와 100ms 순단 timeline이 남음. `OPS-007` x86_64/aarch64 native artifact 실행·SBOM·attestation은 AUTO_PASS이며, public cutover·bypass와 `OPS-009` shadow 복구는 기존 VPS 증거가 있으나 현재 서버는 원본 Nginx topology로 복구됨
