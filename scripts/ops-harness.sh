...
bash scripts/restore-g7devops.sh --plan >target-evidence/deployment-restore-plan.txt
...
grep -Fq 'VPSGuard-owned binary, unit, drop-in, config, token' target-evidence/deployment-restore-plan.txt
